...
my ($mnt, $ctor, @args) = @ARGV;
my $h = eval { Data::Reservoir::Shared->$ctor("$mnt/x.shm", @args) };
print $h ? "created\n" : "refused: $@";
P
close $fh;
...
for my $ctor (qw(new new_weighted)) {
    my ($sig, $out) = on_small_tmpfs('', $ctor, 100, 64);
    is $sig, 0, "$ctor: a segment that fits: no signal";
    like $out, qr/^created/, '  and it is created';

    ($sig, $out) = on_small_tmpfs('DATA_RESERVOIR_SHARED_SPARSE=0', $ctor, 100, 64);
    is $sig, 0, "$ctor: DATA_RESERVOIR_SHARED_SPARSE=0: a segment that fits: no signal";
    like $out, qr/^created/, '  and it is created';

    ($sig, $out) = on_small_tmpfs('DATA_RESERVOIR_SHARED_SPARSE=0', $ctor, 1 << 16, 256);
    is $sig, 0, "$ctor: DATA_RESERVOIR_SHARED_SPARSE=0: a segment the filesystem cannot hold does not kill the process";
    like $out, qr/^refused: .*No space left on device/, '  it is refused with the reason';
    diag $out unless $out =~ /^refused/;

    # Construction touches only the header and reader slots.
    ($sig, $out) = on_small_tmpfs('', $ctor, 1 << 16, 256);
    is $sig, 0, "$ctor: by default: no signal";
